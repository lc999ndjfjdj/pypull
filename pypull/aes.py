...
def encrypt(m, key):
    ...
    c = []
    for block in blocks:
        c.append(int2hex(block).zfill(16))
    return ''.join(c)
...
def decrypt(c, key):
    ...
    m = []
    for block in blocks:
        m.append(int2hex(block).zfill(16))
    return ''.join(m)
...
def des(m, key, decrypt=False):
    ...
    if decrypt:
        # ciphertext 是 hex 字符串
        hexblocks = [m[i:i+16] for i in range(0, len(m), 16)]
        for hb in hexblocks:
            blocks.append(hex2int(hb))
    else:
        # 明文是整数，按 64bit 分成多个 DES block
        bitlen = m.bit_length()
        num_blocks = (bitlen + 63) // 64
        for i in reversed(range(num_blocks)):
            block = (m >> (i*64)) & 0xFFFFFFFFFFFFFFFF
            blocks.append(block)
...
if __name__ == "__main__":
    M = 0x0123456789ABCDEFFEDCBA9876543210  # 128bit明文（2个64bit块）
    KEY = 0x8ca64de9c1b123a7ed23375a821a8c2d
